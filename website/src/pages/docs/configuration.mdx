...
| `basePath` | `string` | Where the handler is mounted. Default `"/breadcrumb"`. |
...
| `authorize` | `(req) => boolean \| Response` | Guards the dashboard + query routes. Return `true` to allow, `false` for 401, or a `Response` (e.g. a redirect). |
...
| `maxPayloadChars` | `number` | Caps captured input/output size. Default `16384`; `0` disables. |
